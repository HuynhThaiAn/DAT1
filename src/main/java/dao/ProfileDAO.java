...
public boolean updateProfileCustomer(int customerID, String fullName, String phone, String birthDate, String gender) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(sql)) {
        ...
        if (phone == null) ps.setNull(2, java.sql.Types.VARCHAR);
        else ps.setString(2, phone);

        if (birthDate == null) ps.setNull(3, java.sql.Types.DATE);
        else ps.setString(3, birthDate); // yyyy-MM-dd OK

        if (gender == null) ps.setNull(4, java.sql.Types.VARCHAR);
