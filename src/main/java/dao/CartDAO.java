...
public boolean getProductQuantityLeft(int productId, int requiredQuantity) throws SQLException {
    String sql =
        "SELECT " +
        "  ISNULL(SUM(isd.Quantity), 0) - " +
        "  ISNULL((SELECT SUM(od.Quantity) FROM OrderDetails od WHERE od.ProductID = ?), 0) AS StockLeft " +
        "FROM ImportStockDetails isd " +
        "WHERE isd.ProductID = ?";

    try (PreparedStatement ps = conn.prepareStatement(sql)) {
        // tham số 1 cho subquery OrderDetails
        ps.setInt(1, productId);
        // tham số 2 cho WHERE isd.ProductID = ?
        ps.setInt(2, productId);

        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                int stockLeft = rs.getInt("StockLeft");
                return stockLeft >= requiredQuantity;
            }
        }
    }
    // không có dòng nhập nào → coi như hết
    return false;
}
}
