...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const routes = [
            {key: "dashboard", match: ["staffdashboard"]},
            {key: "orders", match: ["vieworderlist", "vieworderdetail", "updateorder"]},
            {key: "stock", match: ["importstatistic"]},
            {key: "customers", match: ["customerlist"]},
            {key: "feedback", match: ["viewlistnewfeedback"]},
        ];
        ...
        const found = routes.find(r => r.match.some(s => path.includes(s)));

        if (found) {
            links.forEach(a => {
                const href = (a.getAttribute("href") || "").toLowerCase();
                if (href.includes(found.match[0].split(/[?#]/)[0]) || found.match.some(m => href.includes(m))) {
                    a.classList.add("active");
                }
            });

            
            if (![...links].some(a => a.classList.contains("active"))) {
                const mapHref = {
                    dashboard: "staffdashboard",
                    orders: "vieworderlist",
                    stock: "importstatistic",
                    customers: "customerlist",
                    feedback: "viewlistnewfeedback"
                };
                links.forEach(a => {
                    const href = (a.getAttribute("href") || "").toLowerCase();
                    if (href.includes(mapHref[found.key]))
                        a.classList.add("active");
                });
            }
        } else {
